            ...
            <p class="page-copy">
                <c:choose>
                    <c:when test="${isCatalogSection}">
                        Bạn đang ở tab <strong>Quản lý danh sách kỹ năng</strong>. Tab này chỉ hiển thị dữ liệu danh mục kỹ năng và không trộn với phần quản lý kỹ thuật viên.
                    </c:when>
                    <c:otherwise>
                        Bạn đang ở tab <strong>Quản lý danh sách kỹ thuật viên</strong>. Tab này chỉ hiển thị dữ liệu kỹ thuật viên, hồ sơ điều phối, kỹ năng được gán và khoảng bận.
                    </c:otherwise>
                </c:choose>
            </p>
        </div>
                                    ...
                                    <div class="inner-card">
                                        ...
                                        <div class="inner-card-body">
                                            <form method="post" action="${pageContext.request.contextPath}/manager/technician-capability" class="row g-3 mb-4">
                                                ...
                                                <input type="hidden" name="technicianId" value="${selectedTechnicianId}">
                                                <input type="hidden" name="section" value="technicians">
                                                <div class="col-md-12"><label class="form-label">Chọn kỹ năng</label><select class="form-select" name="skillCode" required><option value="">-- Chọn kỹ năng --</option><c:forEach items="${skillCatalog}" var="skill"><c:if test="${skill.activeStatus}"><option value="${skill.code}">${skill.code} - ${skill.name}</option></c:if></c:forEach></select></div>
