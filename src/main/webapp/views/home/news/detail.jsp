          ...
          <article class="article-card">
            <c:choose>
              ...
              <c:when test="${not empty news.imageUrl and (fn:startsWith(news.imageUrl, '/') or fn:startsWith(news.imageUrl, 'uploads/') or fn:startsWith(news.imageUrl, 'news-images/'))}">
                <img src="${ctx}${fn:startsWith(news.imageUrl, '/') ? '' : '/'}${news.imageUrl}" class="article-cover" alt="${news.title}">
              </c:when>
              <c:when test="${not empty news.imageUrl}">
                <img src="${ctx}/uploads/news-images/${news.imageUrl}" class="article-cover" alt="${news.title}">
              </c:when>
              <c:otherwise>
                ...
                <c:forEach items="${relatedNews}" var="r">
                  <a href="${ctx}/news/detail?id=${r.id}" class="related-item">
                    <c:choose>
                      ...
                      <c:when test="${not empty r.imageUrl and (fn:startsWith(r.imageUrl, '/') or fn:startsWith(r.imageUrl, 'uploads/') or fn:startsWith(r.imageUrl, 'news-images/'))}">
                        <img src="${ctx}${fn:startsWith(r.imageUrl, '/') ? '' : '/'}${r.imageUrl}" class="related-thumb" alt="${r.title}">
                      </c:when>
                      <c:when test="${not empty r.imageUrl}">
                        <img src="${ctx}/uploads/news-images/${r.imageUrl}" class="related-thumb" alt="${r.title}">
                      </c:when>
                      <c:otherwise>
