...
<%-- 1. Lấy user từ session để đồng bộ trạng thái đăng nhập --%>
...
<head>
  ...
  <style>
    ...
    .btn-white { background-color: white; color: #4e73df; font-weight: 700; padding: 10px 25px; border-radius: 50px; transition: 0.3s; text-decoration: none; border: none; }
    .btn-white:hover { transform: translateY(-3px); box-shadow: 0 10px 20px rgba(0,0,0,0.2); color: #224abe; }

    /* --- CHỈNH LẠI HEADER CHO PHÙ HỢP --- */
    .about-header {
      background: linear-gradient(135deg, var(--primary-color) 0%, var(--secondary-color) 100%);
      color: white;
      padding: 150px 0 100px; /* Tăng padding-top để nội dung không bị Navbar đè lên */
      text-align: center;
    ...
    .info-card { background: white; border: none; border-radius: 15px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); height: 100%; transition: 0.3s; }
    .info-card:hover { transform: translateY(-5px); }
    .entity-icon { width: 60px; height: 60px; background: #eef2fd; color: var(--primary-color); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; margin: 0 auto 20px; }
    .system-highlight { border-left: 5px solid var(--primary-color); background: #fff; padding: 25px; border-radius: 0 15px 15px 0; box-shadow: 0 5px 15px rgba(0,0,0,0.05); }
