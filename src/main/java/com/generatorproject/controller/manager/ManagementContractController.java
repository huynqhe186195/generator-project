...
public class ManagementContractController extends HttpServlet {
    ...
    private void showDetail(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            List<Product> customerDevices = productServices.getAllProductByCustomerId(contract.getCustomerId());
            ...
            req.setAttribute("customerDevices", customerDevices);
    ...
    private void handleImportFile(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        try {
            Users manager = (Users) req.getSession().getAttribute("USERMODEL");
            if (manager == null) {
            ...
            if (msg != null && msg.startsWith("MISSING_USER|")) {
                ...
                for (String part : parts) {
                    ...
                    if (part.startsWith("fullName="))
                        fullName = part.substring("fullName=".length());
                    if (part.startsWith("phoneNumber="))
                        phone = part.substring("phoneNumber=".length());
                }
                ...
                req.setAttribute("errorMessage",
                        "Email '" + email
                                + "' chưa có tài khoản hệ thống. Bạn có muốn gửi yêu cầu Admin tạo mới không?");
                showList(req, resp);
                return;
            }
            ...
            if (serialNumbers != null && serialNumbers.length > 0) {
                for (int i = 0; i < serialNumbers.length; i++) {
                    ...
                    if (inputModelName == null || inputModelName.trim().isEmpty()) {
                        req.setAttribute("errorMessage",
                                "Vui lòng nhập tên máy phát điện cho serial " + serialNumber + "!");
                        showEditForm(req, resp);
                        return;
                    }
                    ...
                    if (model == null) {
                        req.setAttribute("errorMessage", "Lỗi: Dòng máy '" + inputModelName
                                + "' chưa có trong hệ thống! (Serial " + serialNumber + ")");
                        showEditForm(req, resp);
                        return;
                    }
                ...
                if (model == null) {
                    req.setAttribute("errorMessage",
                            "Lỗi: Dòng máy '" + inputModelName + "' chưa có trong hệ thống! Vui lòng chọn từ gợi ý.");
                    showEditForm(req, resp);
                    return;
                }
