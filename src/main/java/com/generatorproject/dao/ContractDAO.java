...
public class ContractDAO extends GenericDAO<Contract> {
    ...
    public boolean isContractNumberExists(String contractNumber) {
        String sql = "SELECT count(*) FROM contracts WHERE contract_number = ? AND deleted_at IS NULL";
        int count = count(sql, contractNumber);
        return count > 0;
    }

    public Contract findByContractNumber(String contractNumber) {
        String sql = "SELECT * FROM contracts WHERE contract_number = ? AND deleted_at IS NULL";
    ...
    public void delete(Long id) {
        String sql = "UPDATE contracts SET deleted_at = NOW() WHERE id = ?";
        update(sql, id);
    }

    public List<Contract> findByProductId(Long productId) {
        String sql = "SELECT * FROM contracts WHERE product_id = ? AND deleted_at IS NULL";
        return query(sql, new ContractMapper(), productId);
    }

    public List<Contract> getContractByCustomerId(int id) {
        String sql = "SELECT * FROM contracts WHERE customer_id = ? AND deleted_at IS NULL";
        return query(sql, new ContractMapper(), id);
    }

    // Phần tổng quan
    public int countByStatus(String status) {
        String sql = "SELECT COUNT(*) FROM contracts WHERE status = ? AND deleted_at IS NULL";
